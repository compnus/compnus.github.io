...
Deno.serve(async (req) => {
    ...
    try {
        ...
        if (!sendData || sendError) {
            return new Response(JSON.stringify({ response: "There was a problem updating your balance." + amount + " " + parseInt(amount) + " " + parseInt((await supabase.from("variable").select("value").eq("key", (btc ? "nocaforsat" : "nocafornus")).single()).value) }), {
                status: 400,
                headers: {
                    ...headers
                }
