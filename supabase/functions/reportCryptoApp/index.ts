...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const { error: logError } = await sb
            .from("logs")
            .insert([{ created_by: from, type: "submission_report", attributes: "id->" + app + "; type->"+type + (type=="earn"?("; oft->"+oft):""), message: dsc }]);
        if (logError) {
        ...
        return new Response(JSON.stringify({ response: "App reported successfully!<br>Thank you!", sc: true }), {
            status: 200,
            headers: {
                ...headers
            }
