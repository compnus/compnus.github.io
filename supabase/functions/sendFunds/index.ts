    ...
    try {
        ...
        if (userExistsErrorc || !senuserc) {
            ...
            const { error: logError2 } = await sb
                .from("transaction")
                .insert({ from: from, to: "CompNUS", resource: resources, message: "Transaction Fee", expiration: 1 });
            ...
            let upmessage = { from: "CompNUS", subject: `You have received ${currency==="nus"?totalReceived.toLocaleString('en-US', { useGrouping: false, maximumSignificantDigits: 21 }):totalReceived } <span style="font-family: 'currencycompnus',Ubuntu !important; font-weight: normal !important;">${currencyThing}</span> from ${from}!`, content: `<p>The amount has been added to your balance.${finalMessage}<br><i>Something wrong? <a class="link" href="transactions.html?report=${logData[0].id}">Report Transaction</a></i></p>`, owner: bData.id};
